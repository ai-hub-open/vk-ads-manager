...
# VK Ads Manager — полный lifecycle
...
- `scripts/setup_vk_ads_mcp.py` — **подключает хостовый vk-ads-mcp** (https://vkads-mcp.aihub.click.ru/mcp, 45 инструментов) к Cursor / Claude Code / Claude Desktop: токен click.ru + ID аккаунта (либо готовая персональная ссылка подключения — `--connection-url`, заголовки тогда не нужны), `--target`, `--dry-run`, `--remove`. Клонировать репозиторий и ставить Bun не нужно. Требует запуска кода; без него — конфиг пишет человек по `docs/hosted-mcp-setup.md`. По пути click.ru заодно (а) подключает коннектор `keepimage` для заливки картинок (`--no-keepimage` отключает) и (б) сохраняет токен в реестр ключей (`clickru`), поэтому `upload_creatives_to_storage.py` готов сразу — отдельный `manage_credentials set clickru` не нужен. Путь через MCP — **primary** для Шага 11 (приоритет над нашим REST). См. `references/vk-ads-mcp-integration.md`.
...
Читаем сайты топ-5 (инструмент чтения веб-страниц, например `WebFetch`). **Нечем читать** — разбор строится на том, что знает клиент, плюс на скриншотах, которые он пришлёт; пункты, которые так не добываются (цены, Tone of Voice), помечай `n/a`, а не догадкой, и ставь пропуск в `04_competitor_analysis.md`. Для каждого:
- Офер (H1)
- УТП
- Цены
...
- Инструкцию: где взять `pixel_id` в кабинете → куда вставить → как проверить (`vk_pixel_check.html`)
...
- **Дал публичные ссылки** — запиши их креативам в `creatives.json` полями `image_url` (карусель — `image_urls`) и `video_url`; на Шаге 11 (пре-флайт п.1) они проверяются HEAD-запросом и идут в `vk_ads_content_upload_image` напрямую, без KeepImage. Путь B (`deploy_campaign.py`) читает те же поля и скачивает ссылку сам.
...
2. **После «ОК» — реальный запуск**:
   ...
   ```
   Что произойдёт:
   - Загрузит медиа по каждому креативу. Источник ищется в таком порядке:
     `image_url` / `video_url` / `image_urls` из `creatives.json` (публичная ссылка от
     маркетолога — приоритет: его картинка заменяет нашу генерацию с тем же именем) →
     локальный файл `assets/images/<creative_name>.png` или `assets/videos/<creative_name>.mp4`.
     Ссылку скачивает сам скрипт и отправляет байты: VK по ссылке не ходит, эндпоинт
     принимает только multipart. KeepImage пути B не нужен — локальные файлы он читает сам.
   - Создаст ad_plan — «Кампанию» верхнего уровня (status=blocked)
   - Создаст группы вместе с объявлениями: **объявления уходят вложенным массивом
     `banners` внутри payload группы** — отдельного создания объявления в API нет
     (`POST /banners.json` → `405`), и «долить» их в готовую группу невозможно (всё blocked)
   - Запишет в `operations_log.md`
   - Сохранит `assets/deploy_plan.json` со всеми ID

   ⚠️ Если по креативу нет ни ссылки, ни файла — объявление не соберётся, и группа без
   объявлений будет пропущена. Не осталось ни одной группы — залив прерывается с ошибкой,
   а не создаёт пустую кампанию: группа без баннеров мертва (`NO_BANNERS_WITH_ACTIVE_STATUS`).
