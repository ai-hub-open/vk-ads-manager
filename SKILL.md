...
- `scripts/setup_vk_ads_mcp.py` — **подключает хостовый vk-ads-mcp** (https://vkads-mcp.aihub.click.ru/mcp, 45 инструментов) к Cursor / Claude Code / Claude Desktop: токен click.ru + ID аккаунта, `--target`, `--dry-run`, `--remove`. Клонировать репозиторий и ставить Bun не нужно. Требует запуска кода; без него — конфиг пишет человек по `docs/hosted-mcp-setup.md`. По пути click.ru заодно (а) подключает коннектор `keepimage` для заливки картинок (`--no-keepimage` отключает) и (б) сохраняет токен в реестр ключей (`clickru`), поэтому `upload_creatives_to_storage.py` готов сразу — отдельный `manage_credentials set clickru` не нужен. Путь через MCP — **primary** для Шага 11 (приоритет над нашим REST). См. `references/vk-ads-mcp-integration.md`.
