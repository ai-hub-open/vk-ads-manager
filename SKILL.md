...
3. **Проверь, есть ли у тебя инструменты MCP-сервера `vk-ads`** (в одной среде они видны как `mcp__vk-ads__*`, в другой имена выглядят иначе — смотри по короткому имени, например `create_ad_plan`). Есть — это primary путь для финального залива (Шаг 11), скажи об этом пользователю. Нет — предложи установить через `scripts/setup_vk_ads_mcp.py`. Не блокирующее: без MCP скилл работает через наш REST, а без него — ручным заливом.
...
- `scripts/setup_vk_ads_mcp.py` — **подключает vk-ads-mcp** (https://github.com/ai-hub-open/vk-ads-mcp, 48 инструментов). **Умеет только конфиг Claude Desktop:** обновляет `claude_desktop_config.json`, после рестарта приложения инструменты сервера становятся доступны в чате. Требует запуска кода. Среда другая — подключение делает человек, скилл его не настраивает. Путь через MCP — **primary** для Шага 11 (приоритет над нашим REST). См. `references/vk-ads-mcp-integration.md`.
...
1. **Собери базу из кабинета (MCP).** Если есть доступ (инструменты MCP-сервера `vk-ads` — например, с префиксом `mcp__vk-ads__` — либо `api_available`) — вытащи фактические **CPM и CTR** через `get_statistics_summary` / `get_statistics_day` по похожим/прошлым кампаниям клиента (из аудита Шага 0.5). Это тег `api`. Нет доступа или нет похожих кампаний → CPM/CTR только `benchmark`, и так и пометь; если причина в среде, а не в отсутствии истории, добавь пометку о пропуске в `_forecast.md`.
...
**Если инструменты MCP-сервера `vk-ads` есть** (в одной среде они видны с префиксом `mcp__vk-ads__`, в другой имена выглядят иначе — ищи по короткому имени: `create_ad_plan`, `upload_image`):
...
**Если MCP-инструментов нет** — предложи установить за 5 минут:
```
python -m scripts.setup_vk_ads_mcp --vk-ads-mcp-path <путь к клонированному репо>
```
См. `references/vk-ads-mcp-integration.md` для полной инструкции. Установщик умеет только конфиг Claude Desktop и требует запуска кода: среда другая или кода запустить нельзя — подключение делает человек, а ты идёшь на путь B или C, не пытаясь настроить MCP сам.
