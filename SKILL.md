...
Скилл проводит как через 10-шаговую воронку подготовки кампании в VK Рекламе (ads.vk.ru), так и через повседневную работу с уже существующей кампанией. На максимуме автоматизации: сам ищет конкурентов через WebSearch, парсит лендинги, генерит persona, проектирует структуру аудиторий и креативы. Пользователь — source of truth и редактор, не оператор.
...
## ⚠️ Главный режим: «один окно» (Cowork-first)

Скилл рассчитан на **не-технического пользователя в Cowork mode**. Это значит:

- ✅ **Claude сам запускает скрипты** через `mcp__workspace__bash`. Пользователь не открывает терминал.
- ✅ **Claude сам собирает результаты** и показывает превью / итоги в чате (computer:// ссылки на файлы, embed картинок где возможно).
- ✅ **Ключи API пользователь скидывает в чат** — Claude сохраняет через `manage_credentials.py set <service> --key <KEY>`. Способ Б — дефолт для коллег. Способ А (`getpass` в терминале) — только если пользователь явно скажет «я сам».
- ✅ **Каждый Шаг 1-10 + lifecycle** — Claude сам выполняет, не передаёт пользователю команды для копи-паста.
- ❌ **Не передавай пользователю команды для терминала**, если можешь сделать сам через bash. Исключение: если bash недоступен в текущей среде (например ошибка `Workspace unavailable`) — тогда даёшь команду и просишь выполнить.
...
1. Проверь установлены ли зависимости — выполни:
   ...
   ```
   Если ошибка — попроси пользователя один раз запустить `python <skill-path>/install.py` (это идемпотентно, безопасно).

2. Проверь сохранены ли ключи — выполни:
   ```
   python -m scripts.manage_credentials list
   ```
   Если каких-то нужных нет — попросишь когда дойдёшь до шага где они нужны (не заранее всех сразу).

3. **Проверь подключен ли vk-ads-mcp** — спроси у себя «есть ли у меня tools `mcp__vk-ads__*`?». Если ДА — это primary путь для финального залива (Шаг 11), упомяни пользователю что мы используем MCP. Если НЕТ — предложи установить через `scripts/setup_vk_ads_mcp.py` для упрощения работы. Не блокирующее — без MCP скилл тоже работает через наш REST.
...
- `references/vk-ads-mcp-integration.md` — **рекомендуемый путь работы с VK Ads API через MCP-сервер** vk-ads-mcp (48 инструментов). Включает: установку, конфигурацию Claude Desktop, полный список tools, маппинг наших задач на tools, правильную терминологию (`ad_plan` vs `campaign` vs `banner`). **Обязательно прочитать перед Шагом 11** — если у пользователя подключен MCP, идём по нему вместо нашего REST.
...
- `scripts/setup_vk_ads_mcp.py` — **подключает vk-ads-mcp** (https://github.com/ai-hub-open/vk-ads-mcp, 48 инструментов) к Claude Desktop. Одна команда → обновляет `claude_desktop_config.json` → после рестарта Desktop в чате доступны `mcp__vk-ads__*` tools. Это **primary путь** для Шага 11 в Cowork mode (приоритет над нашим REST). См. `references/vk-ads-mcp-integration.md`.
...
Если у пользователя уже есть токен VK Ads API — попроси выполнить read-only вызов `GET /api/v2/campaigns.json` через `scripts/vk_ads_api.py audit`. Запиши `_account_audit.md`: всего/активных/на паузе кампаний, имена, бюджеты, цели. Пометь семантически близкие.
...
Если токена нет — пользователь работает руками в UI ads.vk.ru, скилл всё равно полезен, просто финальный залив будет через UI (см. Шаг 11).
...
WebFetch на лендинг. **Если заблокирован** — спроси 5 пунктов у юзера: H1, бенефиты, CTA, цена/триал, доказательная база. Пометь что данные со слов.
...
WebFetch. **Артефакт:** `02_visual.md` со ссылками или встроенными фрагментами + явный список **чего не хватает** (визуал — частый блокер на Шаге 10).
...
Собери широкую корзину 10-20 кандидатов (WebSearch по транзакционным запросам И по запросам про заменители «как вести Y в Excel»; `vk.com/search`; каталоги; отзовики/подборки «альтернативы X»; спроси клиента «с кем вас сравнивают / что говорят при отказе»).
...
WebSearch не отдаёт креативы VK дословно. Иерархия источников:
...
3. **Реконструкция** через WebSearch + ленты сообществ конкурентов. Тег `source-trust: reconstruction`.
...
WebFetch на лендинг. Если снова заблокирован — описание из брифа + дисклеймер.
...
1. **Собери базу из кабинета (MCP).** Если есть доступ (`mcp__vk-ads__*` или `api_available`) — вытащи фактические **CPM и CTR** через `get_statistics_summary` / `get_statistics_day` по похожим/прошлым кампаниям клиента (из аудита Шага 0.5). Это тег `api`. Нет доступа/похожих → CPM/CTR только `benchmark`, и так и пометь.
...
**Главное правило:** скилл должен работать «в одно окно» — пользователь не открывает терминал, не запускает команды, не редактирует файлы. Всё происходит через диалог с Claude в Cowork mode.

Стратегия запроса ключа (для модели):

1. **Сначала проверь сохранён ли уже ключ.** Выполни сам через `mcp__workspace__bash`:
   ```
   python -m scripts.manage_credentials list
   ```
...
**Покажи варианты пользователю** через computer:// ссылки — пусть выберет лучший.
...
4. **Дай пользователю computer:// ссылку** на PDF, проси проверить. Правки → меняем markdown → `--force` для перегенерации.
...
| Путь | Когда выбирать | Главный инструмент |
|---|---|---|
| **A. vk-ads-mcp** (рекомендуется) | Cowork mode, MCP-сервер подключен в Claude Desktop | Прямые tool-вызовы `mcp__vk-ads__*` |
| **B. Наш REST API** (M4) | Cowork без MCP, либо автоматизация через cron | `scripts/deploy_campaign.py` |
| **C. UI fallback** (M5) | Нет API доступа / клиент не выдаёт токен | `scripts/generate_launch_guide.py` |

### Путь A: vk-ads-mcp (рекомендуемый для Cowork)

**Если MCP уже подключен** (проверь: спроси Claude — есть ли у него tools начинающиеся с `mcp__vk-ads__`):

1. **Загрузка медиа:** для каждой картинки/видео — `mcp__vk-ads__upload_image` / `mcp__vk-ads__upload_video`. Сохраняй возвращённые ID.
2. **Создание ad_plan (UI «Кампания», верхний уровень):** `mcp__vk-ads__create_ad_plan` с nested `campaigns: [...]` (по числу аудиторий) и nested `banners: [...]` внутри каждой campaign. Один атомарный вызов — избегаем orphan'ов.
3. **Альтернатива:** `mcp__vk-ads__create_ad_plan` → потом `mcp__vk-ads__create_campaign(ad_plan_id=...)` для каждой группы → `mcp__vk-ads__create_banner(campaign_id=...)` для каждого креатива.
4. **Все объекты в `status: "blocked"`** — никогда не активируй сам.
5. **Проверка:** `mcp__vk-ads__get_ad_plan(id=...)` → покажи пользователю что создано.
6. **Дай ссылку:** `https://ads.vk.ru/hq/campaign/<ad_plan_id>` (в URL это всё ещё `/campaign/`, исторически).

**Если MCP НЕ подключен** — предложи установить за 5 минут:
```
python -m scripts.setup_vk_ads_mcp --vk-ads-mcp-path <путь к клонированному репо>
```
См. `references/vk-ads-mcp-integration.md` для полной инструкции.
...
### Если API недоступен — M5 UI fallback

Если у пользователя/клиента нет токена VK Ads (или клиент не готов выдавать, или API возвращает ошибки) — используй M5:
...
Покажи получившийся гайд через computer:// ссылку. Оператор / маркетолог со стороны клиента сможет залить руками за 1-2 часа.
...
**Перед выдачей** проверь чек-лист согласований в конце отчёта (список конкурентов, УТП по сегментам, прогноз согласован маркетологом, тексты юр.чисты, структура = xlsx). Дай пользователю computer:// ссылки на `strategy_report.docx` и нужные .md.
...
- **Cowork-first.** Все скрипты запускай сам через `mcp__workspace__bash`. Если bash недоступен — только тогда передай команду пользователю с пояснением «к сожалению окружение не дало мне запустить, выполните пожалуйста: ...».
- **Ключи в чате — это нормально.** Когда пользователь скидывает API-ключ — благодари кратко, сразу сохраняй через `manage_credentials.py set <service> --key <KEY>`, в следующих сообщениях не повторяй ключ (используй маску если нужно).
- **Показывай превью.** После генерации картинки / создания кампании — давай computer:// ссылку на файл или встраивай в чат. Пользователь не должен лазить по папкам чтобы увидеть результат.
