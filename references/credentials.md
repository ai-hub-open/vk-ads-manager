...
Скилл рассчитан на то, что **пользователь не открывает терминал**. Поэтому **Способ Б — дефолт.**

**Если в среде нет запуска кода** — хранилища ключей нет вообще: `manage_credentials.py` не выполнить. Тогда ключ не спрашивай заранее и не сохраняй; шаг, которому он нужен, идёт по своей ветке «если нет» с пометкой о пропуске. Просить у пользователя ключ, который некуда положить и нечем использовать, — худший вариант: и данные засветили, и результата нет.

В SKILL.md в каждом шаге, требующем ключ, есть инструкция:

```
Перед запуском <script_name>:
1. Проверь — выполни сам инструментом запуска кода:
   python -m scripts.manage_credentials list
...
Пользователь скидывает ключ в чат → агент сохраняет через `--key`.

- ✅ Не требует от пользователя терминальных навыков
- ✅ Работает в одном окне, без переключения в терминал
- ⚠️ Ключ попадает в историю чата — но не в артефакты проекта

Применять когда: пользователь не-технический и работает только через диалог. Требует способности **запускать код** — без неё способ не работает (см. выше).
