...
> Этот файл — справочная документация по прямому REST. В Cowork можно работать и через `vk-ads-mcp` (см. `references/vk-ads-mcp-integration.md`), и через наш `scripts/vk_ads_api.py` — клиент исправлен и создаёт корректную иерархию (см. ниже).
